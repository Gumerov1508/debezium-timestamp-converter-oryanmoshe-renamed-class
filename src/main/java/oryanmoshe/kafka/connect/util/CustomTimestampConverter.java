...
//    private final SchemaBuilder datetimeSchema = SchemaBuilder.string().optional().name("oryanmoshe.time.DateTimeString");
    ...
    public void configure(Properties props) {
        ...
        if (this.debug)
            System.out.printf(
                    "[CustomTimestampConverter.configure] Finished configuring formats. this.strDatetimeFormat: %s, this.strTimeFormat: %s%n",
    ...
    public void converterFor(RelationalColumn column, ConverterRegistration<SchemaBuilder> registration) {
        if (this.debug)
            System.out.printf(
                    "[CustomTimestampConverter.converterFor] Starting to register column. column.name: %s, column.typeName: %s, column.hasDefaultValue: %s, column.defaultValue: %s, column.isOptional: %s%n",
        ...
        if (SUPPORTED_DATA_TYPES.stream().anyMatch(s -> s.equalsIgnoreCase(column.typeName()))) {
            ...
            registration.register(SchemaBuilder.string().optional(), rawValue -> {
                if (rawValue == null) {
                    // DEBUG
                    if (this.debug) {
                        System.out.printf("[CustomTimestampConverter.converterFor] rawValue of %s is null.%n", column.name());
                    }
                ...
                if (this.debug)
                    System.out.printf(
                            "[CustomTimestampConverter.converterFor] Before returning conversion. column.name: %s, column.typeName: %s, millis: %d%n",
